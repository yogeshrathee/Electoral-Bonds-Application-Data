...
public interface PoliticalPartyRepo extends JpaRepository<PoliticalParty, Integer> {
    ...
    public List<PoliticalParty> findByName(@Param("NAME_OF_THE_POLITICAL_PARTY") String NAME_OF_THE_POLITICAL_PARTY);

    @Query("SELECT SUM(p.DENOMINATIONS) FROM PartyDetails p")
    public int findTotalDenominationByName(String NAME_OF_THE_POLITICAL_PARTY);

    @Query("SELECT u from PartyDetails u where u.DATE_OF_ENCASHMENT=:DATE_OF_ENCASHMENT")
    public List<PoliticalParty> findByDate(@Param("DATE_OF_ENCASHMENT") String DATE_OF_ENCASHMENT);
